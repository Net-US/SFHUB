...
<div class="animate-fade-in-up space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex gap-2">
            ...
            <button onclick="showAddPostModal()" class="flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-sm font-medium transition-colors">
    ...
    <div class="bg-white dark:bg-stone-900 rounded-2xl p-4 border border-stone-200 dark:border-stone-800">
        <div class="flex flex-wrap gap-2 items-center">
            ...
            <button onclick="filterByCategory('all')" class="px-3 py-1.5 rounded-full text-sm {{ !request('category') ? 'bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400' : 'bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300' }}">All</button>
            @foreach($categories as $category)
            <button onclick="filterByCategory({{ $category->id }})" class="px-3 py-1.5 rounded-full text-sm flex items-center gap-1 {{ request('category') == $category->id ? 'bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400' : 'bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300' }}">
                    ...
                    <tr class="border-b border-stone-100 dark:border-stone-800 hover:bg-stone-50 dark:hover:bg-stone-800">
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                                ...
                                <img src="{{ $post->featured_image }}" class="w-10 h-10 rounded-lg object-cover">
                                @else
                        ...
                        <td class="py-4 px-6">
                            ...
                            <span class="px-2 py-1 rounded-full text-xs" style="background-color: {{ $post->categories->first()->color ?? '#f57223' }}20; color: {{ $post->categories->first()->color ?? '#f57223' }}">
                        ...
                        <td class="py-4 px-6 text-stone-600 dark:text-stone-400">{{ $post->created_at->format('M d, Y') }}</td>
                        <td class="py-4 px-6">
                            <div class="flex gap-2">
                                <button onclick="editPost({{ $post->id }})" class="text-primary-600 hover:text-primary-800 dark:text-primary-400" title="Edit">
                                    <i class="fa-solid fa-edit"></i>
                                </button>
                                <a href="/blog/{{ $post->slug }}" target="_blank" class="text-stone-500 hover:text-stone-700 dark:text-stone-400" title="Preview">
                                    <i class="fa-solid fa-eye"></i>
                                </a>
                                <button onclick="deletePost({{ $post->id }})" class="text-rose-500 hover:text-rose-700 dark:text-rose-400" title="Delete">
    ...
    <div class="bg-white dark:bg-stone-900 rounded-2xl p-6 border border-stone-200 dark:border-stone-800">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.blog.comments') }}" class="text-sm text-primary-600 hover:text-primary-700">View All</a>
        </div>

        <div class="space-y-4">
            @forelse($recentComments ?? [] as $comment)
            <div class="flex items-start gap-4 p-4 border border-stone-200 dark:border-stone-700 rounded-lg">
                <div class="w-10 h-10 rounded-full bg-stone-200 dark:bg-stone-700 flex items-center justify-center text-stone-500 font-medium">
                    {{ substr($comment->author_name ?? $comment->user?->name, 0, 1) }}
                </div>
                <div class="flex-1">
                    <div class="flex justify-between items-start mb-1">
                        <div>
                            <span class="font-medium text-stone-800 dark:text-white">{{ $comment->author_name ?? $comment->user?->name }}</span>
                    ...
                    <p class="text-sm text-stone-600 dark:text-stone-400 mb-2">{{ Str::limit($comment->content, 100) }}</p>
                    <div class="flex gap-2">
                        <span class="px-2 py-1 rounded-full text-xs 
                            ...
                            @else bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400
                            @endif">
                            {{ $comment->status }}
                        </span>
                        @if($comment->status === 'pending')
                        <button onclick="approveComment({{ $comment->id }})" class="text-xs text-primary-600 hover:text-primary-700">Approve</button>
                        @endif
                        <button onclick="deleteComment({{ $comment->id }})" class="text-xs text-rose-600 hover:text-rose-700">Delete</button>
                    </div>
                </div>
            </div>
            @empty
