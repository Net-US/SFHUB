...
<div class="space-y-5">
    ...
    <form method="GET" action="{{ route('admin.users') }}" class="flex gap-2 flex-wrap">
        <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama, email, atau username..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
                    ...
                    <tr class="hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors">
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center gap-2">
                                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-orange-400 to-rose-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                        ...
                        <td class="py-3 px-4"><span class="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400">{{ ucfirst($u->role ?? 'user') }}</span></td>
                        <td class="py-3 px-4"><span class="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-stone-100 dark:bg-stone-700 text-stone-600 dark:text-stone-300">{{ ucfirst($u->plan ?? 'free') }}</span></td>
                        <td class="py-3 px-4">
                            <span class="px-2 py-0.5 rounded-full text-[11px] font-semibold {{ $u->is_active ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400' }}">
                                {{ $u->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="py-3 px-4 text-stone-400 whitespace-nowrap">{{ $u->created_at->format('d M Y') }}</td>
                        <td class="py-3 px-4">
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="text-center py-10 text-stone-400">Tidak ada pengguna ditemukan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
