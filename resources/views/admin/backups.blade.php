...
<div class="animate-fade-in-up space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex gap-2">
            <button onclick="showBackupConfigModal()" class="flex items-center gap-2 px-4 py-2 border border-stone-300 dark:border-stone-700 hover:bg-stone-50 dark:hover:bg-stone-800 text-stone-700 dark:text-stone-300 rounded-xl text-sm font-medium transition-colors">
                <i class="fa-solid fa-gear"></i> Config
            </button>
            <button onclick="createBackup()" class="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-medium transition-colors">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-stone-900 rounded-xl p-4 border border-stone-200 dark:border-stone-800">
            ...
            <p class="text-2xl font-bold text-blue-600">{{ $totalSize ?? '0 MB' }}</p>
        </div>
        <div class="bg-white dark:bg-stone-900 rounded-xl p-4 border border-stone-200 dark:border-stone-800">
            <p class="text-sm text-stone-500 dark:text-stone-400">Last Backup</p>
            <p class="text-2xl font-bold text-emerald-600">{{ $lastBackup ? $lastBackup->created_at->diffForHumans() : 'Never' }}</p>
        </div>
        <div class="bg-white dark:bg-stone-900 rounded-xl p-4 border border-stone-200 dark:border-stone-800">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- Storage Info -->
        <div class="lg:col-span-2 bg-white dark:bg-stone-900 rounded-2xl p-6 border border-stone-200 dark:border-stone-800">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-medium">{{ $dbBackupsCount ?? 0 }} files ({{ $dbBackupsSize ?? '0 MB' }})</span>
                </div>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-medium">{{ $fileBackupsCount ?? 0 }} files ({{ $fileBackupsSize ?? '0 MB' }})</span>
                </div>
                <div class="w-full bg-stone-200 dark:bg-stone-700 rounded-full h-2">
        ...
        <div class="bg-white dark:bg-stone-900 rounded-2xl p-6 border border-stone-200 dark:border-stone-800">
            ...
            <div class="space-y-3">
                <button onclick="backupDatabase()" class="w-full flex items-center gap-3 p-3 border border-stone-300 dark:border-stone-700 rounded-xl hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors">
                ...
                <button onclick="backupFiles()" class="w-full flex items-center gap-3 p-3 border border-stone-300 dark:border-stone-700 rounded-xl hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors">
    ...
    <div class="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 overflow-hidden">
        <div class="p-4 border-b border-stone-200 dark:border-stone-700 flex justify-between items-center">
            ...
            <div class="flex gap-2">
                <select id="type-filter" onchange="filterByType(this.value)" class="px-3 py-2 border border-stone-300 dark:border-stone-700 rounded-lg text-sm dark:bg-stone-800 dark:text-white">
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-stone-100 dark:border-stone-800 hover:bg-stone-50 dark:hover:bg-stone-800">
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg bg-stone-100 dark:bg-stone-800 flex items-center justify-center">
                                    <i class="fa-solid fa-{{ $backup->type === 'database' ? 'database' : ($backup->type === 'files' ? 'folder' : 'archive') }} text-stone-400"></i>
                                </div>
                                <div>
                                    <p class="font-medium text-stone-800 dark:text-white">{{ $backup->name }}</p>
                        ...
                        <td class="py-4 px-6">{{ $backup->size_formatted }}</td>
                        <td class="py-4 px-6">
                            <span class="px-2 py-1 rounded-full text-xs font-medium 
                        ...
                        <td class="py-4 px-6 text-stone-600 dark:text-stone-400">{{ $backup->created_at->format('Y-m-d H:i') }}</td>
                        <td class="py-4 px-6">
                            <div class="flex gap-2">
                                <button onclick="downloadBackup({{ $backup->id }})" class="text-primary-600 hover:text-primary-800 dark:text-primary-400" title="Download">
                                    <i class="fa-solid fa-download"></i>
                                </button>
                                <button onclick="restoreBackup({{ $backup->id }})" class="text-amber-600 hover:text-amber-800 dark:text-amber-400" title="Restore">
                                    <i class="fa-solid fa-rotate-left"></i>
                                </button>
                                <button onclick="deleteBackup({{ $backup->id }})" class="text-rose-600 hover:text-rose-800 dark:text-rose-400" title="Delete">
...
<script>
    ...
    function downloadBackup(id) {
        window.open(`{{ url('admin/backups') }}/${id}/download`, '_blank');
    }

    // Restore backup
    function restoreBackup(id) {
        if (!confirm('WARNING: Restoring this backup will replace current data. Continue?')) return;
