...
@section('styles')
    <style>
        .admin-stats-card {
            @apply bg-white dark:bg-stone-800 rounded-xl p-6 border border-stone-200 dark:border-stone-700 shadow-sm hover:shadow-md transition-shadow;
        }

        .admin-stats-number {
            @apply text-2xl font-bold text-stone-900 dark:text-white;
        }
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-stone-900 dark:text-white">Admin Dashboard</h1>
                <p class="text-stone-600 dark:text-stone-400 text-sm mt-0.5">Kelola sistem dan pantau performa platform SFHUB
                </p>
            </div>
            <div class="flex items-center space-x-3">
                ...
                <a href="{{ route('admin.landing') }}"
                    class="flex items-center gap-2 px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-xl text-sm font-medium transition-colors">
                    <i class="fa-solid fa-palette text-xs"></i> Kelola Landing
                </a>
            </div>
        </div>
        ...
        @if (session('success'))
            <div
                class="p-4 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-xl text-sm border border-emerald-200 dark:border-emerald-800">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            @foreach ([[$totalUsers, 'Total User', 'fa-users', 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400', '+' . $newThisMonth . ' bulan ini'], [$activeUsers, 'User Aktif', 'fa-user-check', 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400', $totalUsers > 0 ? round(($activeUsers / $totalUsers) * 100, 1) . '%' : '0%'], [$totalTasks, 'Total Tasks', 'fa-tasks', 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400', $completedTasks . ' selesai'], [$totalWorkspaces, 'Workspaces', 'fa-layer-group', 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400', $totalFinanceAccounts . ' akun keuangan']] as $stat)
                <div class="admin-stats-card">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="admin-stats-number">{{ $stat[0] }}</p>
                            <p class="admin-stats-label">{{ $stat[1] }}</p>
                            <p class="text-xs text-stone-500 dark:text-stone-400 mt-1">{{ $stat[4] }}</p>
                        </div>
                        <div class="admin-icon-wrapper {{ $stat[3] }}">
                    ...
                    @foreach ($monthlyGrowth as $growth)
                        <div class="flex flex-col items-center flex-1">
                            <div class="w-full bg-blue-500 dark:bg-blue-400 rounded-t"
                                style="height: {{ $growth['users'] > 0 ? min(($growth['users'] / max(array_column($monthlyGrowth, 'users'))) * 100, 100) : 0 }}%; min-height: 20px;">
                            </div>
            ...
            <div class="chart-container">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="text-sm font-medium text-emerald-600 dark:text-emerald-400">{{ $completedTasks }}</span>
                    </div>
                    ...
                    <div class="w-full bg-stone-200 dark:bg-stone-700 rounded-full h-2">
                        <div class="bg-blue-500 h-2 rounded-full"
                            style="width: {{ $totalTasks > 0 ? ($doingTasks / $totalTasks) * 100 : 0 }}%"></div>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-6">
                ...
                <div class="space-y-3">
                    @foreach ([['Admin', $adminCount, 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400'], ['Student', $studentCount, 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'], ['Freelancer', $freelanceCount, 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400']] as $role)
            ...
            <div class="bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-6">
                ...
                <div class="space-y-3">
                    @foreach ([['Free', $freeCount, 'bg-gray-100 dark:bg-gray-900/30 text-gray-600 dark:text-gray-400'], ['Pro', $proCount, 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400'], ['Team', $teamCount, 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400']] as $plan)
                        <div class="flex justify-between items-center">
            ...
            <div class="bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span
                            class="text-sm font-medium text-orange-600 dark:text-orange-400">{{ $totalTransactions }}</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="space-y-4">
                        @foreach ($recentUsers as $recentUser)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    <div
                                        class="w-10 h-10 bg-gradient-to-br from-stone-400 to-stone-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                    ...
                    <div>
                        ...
                        <dl class="space-y-1">
                            ...
                            <div class="flex justify-between">
                                ...
                                <dd class="text-sm font-medium text-stone-900 dark:text-white">{{ config('app.env') }}
                                </dd>
                            </div>
                        </dl>
                    </div>
                    <div>
                        ...
                        <dl class="space-y-1">
                            <div class="flex justify-between">
                                ...
                                <dd class="text-sm font-medium text-stone-900 dark:text-white">
                                    {{ config('database.default') }}</dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-sm text-stone-600 dark:text-stone-400">Database:</dt>
                                <dd class="text-sm font-medium text-stone-900 dark:text-white">
                                    {{ config('database.connections.mysql.database') }}</dd>
                            </div>
                            <div class="flex justify-between">
