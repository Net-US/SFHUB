...
<div class="animate-fade-in-up space-y-8">
        ...
        <div class="bg-white dark:bg-stone-900 rounded-2xl p-5 border border-stone-200 dark:border-stone-800 shadow-sm">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <h3 class="text-xl font-bold text-stone-800 dark:text-white">{{ number_format($stats['users']['total'] ?? 0) }}</h3>
                    <p class="text-xs text-emerald-600 dark:text-emerald-400 mt-1">+{{ $stats['users']['new_today'] ?? 0 }} hari ini</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-stone-900 rounded-2xl p-5 border border-stone-200 dark:border-stone-800 shadow-sm">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <h3 class="text-xl font-bold text-stone-800 dark:text-white">{{ number_format($stats['users']['by_plan']['pro'] ?? 0) }}</h3>
                    <p class="text-xs text-stone-400 mt-1">{{ $stats['users']['total'] > 0 ? round(($stats['users']['by_plan']['pro'] / $stats['users']['total']) * 100, 1) : 0 }}% of total</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-stone-900 rounded-2xl p-5 border border-stone-200 dark:border-stone-800 shadow-sm">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <h3 class="text-xl font-bold text-stone-800 dark:text-white">Rp {{ number_format($stats['revenue']['month_to_date'] ?? 0, 0, ',', '.') }}</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white dark:bg-stone-900 rounded-2xl p-6 shadow-sm border border-stone-200 dark:border-stone-800">
            <div class="flex justify-between items-center mb-4">
                ...
                <span class="text-xs bg-stone-100 dark:bg-stone-800 px-2 py-1 rounded text-stone-500">Last 6 Months</span>
            </div>
        ...
        <div class="bg-white dark:bg-stone-900 rounded-2xl p-6 shadow-sm border border-stone-200 dark:border-stone-800">
            <div class="flex justify-between items-center mb-4">
                ...
                <span class="text-xs bg-stone-100 dark:bg-stone-800 px-2 py-1 rounded text-stone-500">Total: {{ number_format($stats['users']['total'] ?? 0) }}</span>
            </div>
            <div class="chart-container h-64">
    ...
    <div class="bg-white dark:bg-stone-900 rounded-2xl p-6 shadow-sm border border-stone-200 dark:border-stone-800">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('admin.users') }}" class="text-sm text-primary-600 hover:text-primary-700 dark:text-primary-400">View All →</a>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                    <tr class="text-left text-xs text-stone-500 dark:text-stone-400 border-b border-stone-200 dark:border-stone-700">
                    ...
                    <tr class="border-b border-stone-100 dark:border-stone-800 last:border-0">
                        <td class="py-3">
                            <div class="flex items-center space-x-3">
                                <div class="w-8 h-8 rounded-full bg-gradient-to-tr from-primary-400 to-primary-600 flex items-center justify-center text-white text-xs font-bold">
    ...
    <div class="bg-white dark:bg-stone-900 rounded-2xl p-6 shadow-sm border border-stone-200 dark:border-stone-800">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('admin.logs.index') }}" class="text-sm text-primary-600 hover:text-primary-700 dark:text-primary-400">View All Logs →</a>
        </div>
        <div class="space-y-3">
...
<script>
    ...
    new Chart(userDistCtx, {
        ...
        data: {
            ...
            datasets: [{
                data: [{{ $stats['users']['by_plan']['free'] ?? 0 }}, {{ $stats['users']['by_plan']['basic'] ?? 0 }}, {{ $stats['users']['by_plan']['pro'] ?? 0 }}],
