            ...
            @if ($actNow)
                <div class="mx-5 mb-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-xl border-2"
                    ...
                    <div class="flex-1 min-w-0">
                        <p
                            class="font-bold text-stone-800 dark:text-white text-sm flex flex-col sm:flex-row sm:items-center gap-2 flex-wrap">
                            {{ $actNow['label'] }}
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div class="space-y-2">
                            ...
                            @forelse($eisenhowerTasks['q1'] ?? collect() as $task)
                                <div
                                    class="p-3 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800">
                                    <p class="text-xs font-medium text-stone-800 dark:text-white truncate">
                                        {{ $task->title }}</p>
                                    @if ($task->due_date)
                                        <p class="text-[10px] text-red-600 dark:text-red-400 mt-1">
                                            {{ $task->due_date->isoFormat('D MMM') }}</p>
                                    @endif
                                </div>
                            @empty
                        ...
                        <div class="space-y-2">
                            ...
                            @forelse($eisenhowerTasks['q2'] ?? collect() as $task)
                                <div
                                    class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
                                    <p class="text-xs font-medium text-stone-800 dark:text-white truncate">
                                        {{ $task->title }}</p>
                                    @if ($task->due_date)
                        ...
                        <div class="space-y-2">
                            ...
                            @forelse($eisenhowerTasks['q3'] ?? collect() as $task)
                                <div
                                    class="p-3 bg-orange-50 dark:bg-orange-900/20 rounded-xl border border-orange-200 dark:border-orange-800">
                                    <p class="text-xs font-medium text-stone-800 dark:text-white truncate">
                                        {{ $task->title }}</p>
                                    @if ($task->due_date)
                        ...
                        <div class="space-y-2">
                            ...
                            @forelse($eisenhowerTasks['q4'] ?? collect() as $task)
                                <div
                                    class="p-3 bg-gray-50 dark:bg-gray-900/20 rounded-xl border border-gray-200 dark:border-gray-800">
                                    <p class="text-xs font-medium text-stone-800 dark:text-white truncate">
                                        {{ $task->title }}</p>
                                    @if ($task->due_date)
                        ...
                        <div class="space-y-3">
                            @foreach ($recommendations as $rec)
                                <div class="flex items-start gap-3 p-3 {{ $rec['cls'] }} rounded-xl">
                                    ...
                                    <div>
                                        <p class="text-xs font-bold text-stone-800 dark:text-white">{{ $rec['title'] }}
                                        </p>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-stone-50 dark:bg-stone-800 border-b border-stone-200 dark:border-stone-700">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium text-stone-600 dark:text-stone-400">
                                Tugas</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-stone-600 dark:text-stone-400">
                                Kategori</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-stone-600 dark:text-stone-400">
                    ...
                    <tbody class="divide-y divide-stone-200 dark:divide-stone-700">
                        @php
                            ...
                            $allTasks = collect(
                                array_merge(
                                    ($eisenhowerTasks['q1'] ?? collect())->all(),
                                    ($eisenhowerTasks['q2'] ?? collect())->all(),
                                    ($eisenhowerTasks['q3'] ?? collect())->all(),
                                    ($eisenhowerTasks['q4'] ?? collect())->all(),
                                ),
                            );
                        ...
                        @forelse($sortedTasks as $task)
                            ...
                            <tr class="hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors">
                                <td class="px-4 py-3">
                                    ...
                                    @if ($task->description)
                                        <p class="text-xs text-stone-500 dark:text-stone-400 mt-0.5">
                                            {{ Str::limit($task->description, 60) }}</p>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $categoryClass }}">
                                        {{ $categoryLabel }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $priorityClass }}">
                                        {{ $priorityLabel }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    ...
                                    @if ($task->due_date)
                                        <p class="text-xs text-stone-500">
                                            {{ $task->due_date->isoFormat('HH:mm') }}</p>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    @if (isset($task->countdown_label))
                                        <span
                                            class="text-xs font-medium {{ $task->countdown['is_overdue'] ?? false ? 'text-red-600' : 'text-amber-600' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center">
                                    ...
                                    <p class="text-sm text-stone-500 dark:text-stone-400">Tidak ada tugas pending
                                    </p>
                                </td>
                            </tr>
                        @endforelse
