...
.fi:focus{border-color:#f43f5e;box-shadow:0 0 0 3px rgba(244,63,94,.1)}
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    @foreach([
        [$completedCount,'Selesai','fa-circle-check','bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600'],
        [$totalCount-$completedCount,'Belum Selesai','fa-clock','bg-rose-100 dark:bg-rose-900/30 text-rose-600'],
        [$totalCount,'Total Tugas','fa-list-check','bg-blue-100 dark:bg-blue-900/30 text-blue-600'],
        [$catCounts->count(),'Kategori','fa-tags','bg-amber-100 dark:bg-amber-900/30 text-amber-600'],
    ] as [$v,$l,$ic,$cls])
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="lg:col-span-2 bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm overflow-hidden">
        ...
        <div class="divide-y divide-stone-100 dark:divide-stone-800" id="gtask-list">
            ...
            <div class="task-row flex items-center gap-4 px-5 py-3.5 {{ $t['done'] ? 'opacity-60' : '' }}"
                data-status="{{ $t['done'] ? 'done' : 'pending' }}"
                ...
                <button onclick="deleteGTask({{ $t['id'] }}, this)"
                    title="Hapus tugas"
    ...
    <div class="space-y-4">
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <div class="w-full bg-stone-100 dark:bg-stone-700 rounded-full h-1.5">
                        <div class="bg-rose-500 h-1.5 rounded-full transition-all" style="width:{{ $pct }}%"></div>
                    </div>
        ...
        <div class="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm p-5">
            ...
            <div class="space-y-2">
                <input type="text" id="qt-title" placeholder="Judul tugas..."
                    class="fi text-sm py-2" onkeydown="if(event.key==='Enter')quickAddGT()">
                ...
                <button onclick="quickAddGT(this)" class="w-full py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-sm font-medium transition-colors">
        ...
        <div class="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm p-5">
            ...
            <div class="flex items-center gap-3 p-2.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl mb-2 last:mb-0">
...
{{-- MODAL: ADD TASK --}}
<div id="modal-add-gtask" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center p-4 flex">
    <div class="bg-white dark:bg-stone-900 rounded-2xl w-full max-w-md shadow-2xl border border-stone-200 dark:border-stone-800">
        ...
        <div class="p-6 space-y-4">
            <div>
                <label class="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-1.5">Judul <span class="text-rose-400">*</span></label>
        ...
        <div class="flex gap-3 px-6 pb-6">
            ...
            <button onclick="saveGTask(this)" class="flex-1 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-sm font-semibold transition-colors">
                <i class="fa-solid fa-plus mr-1.5"></i>Tambah
            </button>
        </div>
    </div>
...
function filterGT(type) {
    const on  = 'px-2.5 py-1 text-[11px] rounded-lg font-medium bg-stone-800 dark:bg-stone-700 text-white transition-colors';
    const off = 'px-2.5 py-1 text-[11px] rounded-lg font-medium bg-stone-100 dark:bg-stone-800 text-stone-500 hover:bg-stone-200 transition-colors';
    ['all','pending','done','today'].forEach(k => { document.getElementById('gtf-'+k).className = k===type ? on : off; });
...
function setLoading(btn, on) {
    if (!btn) return;
    if (on) { btn.disabled=true; btn.style.opacity='.7'; }
...
async function deleteGTask(id, btn) {
    if (!confirm('Hapus tugas ini? Tindakan tidak bisa dibatalkan.')) return;
    setLoading(btn, true);
    try {
...
async function quickAddGT(btn) {
    ...
    if (!title) { toast('Isi judul tugas dulu!', false); return; }
    setLoading(btn, true);
    try {
        const res  = await fetch('{{ route("tasks.store") }}', {
            method:  'POST',
            headers: { 'Content-Type':'application/json', 'X-CSRF-TOKEN':_csrf, 'Accept':'application/json' },
            body:    JSON.stringify({ title, category:cat, priority:prio, due_date:new Date().toISOString().split('T')[0], status:'todo' }),
        });
        const data = await res.json();
        if (data.success || res.ok) {
