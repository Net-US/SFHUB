...
.cal-day{min-height:80px;cursor:pointer;transition:all .15s}
.cal-day:hover{background:#fff7ed}
.dark .cal-day:hover{background:rgba(249,115,22,.07)}
.cal-day.today{background:#fff7ed;border:2px solid #f97316}
.dark .cal-day.today{background:rgba(249,115,22,.1)}
.event-chip{height:17px;font-size:10px;border-radius:4px;padding:1px 4px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;color:#fff;cursor:pointer;line-height:15px}

.tab-btn{display:inline-flex;align-items:center;gap:.45rem;padding:.45rem .9rem;border-radius:.75rem;font-size:.8rem;font-weight:500;color:#78716c;cursor:pointer;border:none;background:none;transition:all .15s}
.tab-btn:hover{background:#f5f5f4;color:#1c1917}
.dark .tab-btn:hover{background:#292524;color:#fafaf9}
.tab-btn.active{background:#fff7ed;color:#ea580c;font-weight:600}
.dark .tab-btn.active{background:rgba(249,115,22,.13);color:#fb923c}

.fi{width:100%;border:1.5px solid #e7e5e4;border-radius:.8rem;padding:.6rem .95rem;font-size:.875rem;background:#fafaf9;color:#1c1917;outline:none;transition:border .18s,box-shadow .18s}
.fi:focus{border-color:#f97316;box-shadow:0 0 0 3px rgba(249,115,22,.11)}
.dark .fi{background:#292524;border-color:#44403c;color:#fafaf9}
.dark .fi:focus{border-color:#f97316;background:#1c1917}
...
/* Error state */
.fi.error{border-color:#ef4444;box-shadow:0 0 0 3px rgba(239,68,68,.11)}
.err-msg{font-size:.7rem;color:#ef4444;margin-top:.25rem;display:none}
.err-msg.show{display:block}

/* Spinner */
.btn-loading{position:relative;pointer-events:none;opacity:.8}
.btn-loading::after{content:'';position:absolute;right:12px;top:50%;transform:translateY(-50%);width:14px;height:14px;border:2px solid rgba(255,255,255,.4);border-top-color:#fff;border-radius:50%;animation:spin .6s linear infinite}
...
@php
    ...
    $catLabels  = ['pkl'=>'PKL','academic'=>'Akademik','creative'=>'Kreatif','finance'=>'Keuangan','health'=>'Kesehatan','personal'=>'Personal','routine'=>'Rutin'];
@endphp
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div>
        ...
        <p class="text-stone-400 text-xs">Kalender terintegrasi — PKL, Kuliah, Deadline, Freelance + Kegiatan Rutin</p>
    </div>
    <div class="flex items-center gap-2 flex-wrap">
        ...
        <a href="{{ route('dashboard.smartcalendar', ['month'=>now()->month,'year'=>now()->year]) }}"
            class="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-sm font-medium transition-colors">Hari Ini</a>
...
<div class="flex gap-1 bg-stone-100 dark:bg-stone-800 p-1 rounded-xl w-fit flex-wrap">
    ...
    <button onclick="switchCalTab('rutin')" id="caltab-rutin" class="tab-btn">
        ...
        <span class="px-1.5 py-0.5 bg-orange-500 text-white text-[10px] font-bold rounded-full">{{ count($recurringActivities) }}</span>
        @endif
    </button>
...
<div class="grid grid-cols-1 xl:grid-cols-4 gap-5">
    <div class="xl:col-span-3 bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm overflow-hidden">
        ...
        <div class="grid grid-cols-7">
            ...
            <div class="border-b border-r border-stone-100 dark:border-stone-800 bg-stone-50/50 dark:bg-stone-800/30 cal-day p-1.5"></div>
            @endfor
            ...
            <div class="cal-day border-b border-r border-stone-100 dark:border-stone-800 p-1 {{ $isToday ? 'today' : '' }} {{ $isWeekend ? 'bg-stone-50/60 dark:bg-stone-800/20' : '' }}"
                ...
                <div class="flex items-center justify-between mb-0.5 px-0.5">
                    <span class="text-xs font-bold {{ $isToday ? 'text-orange-600 dark:text-orange-400' : 'text-stone-700 dark:text-stone-300' }}">{{ $d }}</span>
                    @if($isToday)<span class="w-1.5 h-1.5 rounded-full bg-orange-500"></span>@endif
                </div>
                @foreach(collect($dayRecs)->take(1) as $rec)
                <div class="event-chip mb-0.5 opacity-80" style="background:{{ $rec['color'] }}" title="{{ $rec['title'] }}">
                    {{ $rec['icon'] }} {{ Str::limit($rec['title'], 10) }}
                </div>
                @endforeach
                @foreach($dayEvs->take(2) as $ev)
                <div class="event-chip mb-0.5" style="background:{{ $ev['color'] }}" title="{{ $ev['title'] }}">{{ $ev['title'] }}</div>
                @endforeach
                @if($dayEvs->count() + count($dayRecs) > 3)
                <div class="text-[9px] text-stone-400 pl-0.5">+{{ ($dayEvs->count() + count($dayRecs)) - 3 }} lagi</div>
                @endif
            </div>
            @endfor
    ...
    <div class="space-y-4">
        <div class="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm p-4">
        ...
        <div class="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm p-4">
            ...
            <div class="flex items-start gap-2.5 mb-2.5 last:mb-0">
                <div class="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5" style="background:{{ $dl['color'] }}">
                    <i class="fa-solid fa-calendar-day text-white text-[9px]"></i>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-semibold text-stone-800 dark:text-white truncate">{{ $dl['title'] }}</p>
                    <p class="text-[10px] text-stone-400">{{ \Carbon\Carbon::parse($dl['date'])->isoFormat('D MMM') }} ·
                        @if($diff<0) Terlambat @elseif($diff==0) Hari ini! @elseif($diff==1) Besok @else {{ $diff }} hari @endif
                    </p>
                </div>
                @if($diff >= 0 && $diff <= 3)
                <span class="text-[9px] bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 px-1.5 py-0.5 rounded-full font-bold flex-shrink-0">Segera</span>
                @endif
            </div>
            @empty
...
<div id="calpane-rutin" class="cal-pane hidden space-y-4">
    ...
    @if(count($recurringActivities) === 0)
    <div class="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm p-12 text-center">
        ...
        <p class="text-xs text-stone-400 mb-4">Tambahkan jadwal PKL, kuliah, olahraga, atau kegiatan berulang lainnya</p>
        <button onclick="openModal('modal-add-recurring')"
            ...
            <div class="p-5">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        <div class="min-w-0">
                            <h4 class="font-bold text-stone-800 dark:text-white text-sm truncate">{{ $rec['title'] }}</h4>
                            <span class="text-[10px] px-2 py-0.5 rounded-full font-semibold text-white inline-block mt-0.5"
                                style="background:{{ $rec['color'] }}">{{ $freqLabels[$rec['frequency']] ?? $rec['frequency'] }}</span>
                        </div>
                    </div>
                    <div class="flex gap-1 flex-shrink-0 ml-2">
                        <button onclick="editRecurring({{ $rec['id'] }})"
                            class="w-7 h-7 rounded-lg bg-stone-100 dark:bg-stone-700 text-stone-400 hover:text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 flex items-center justify-center transition-colors" title="Edit">
                            <i class="fa-solid fa-pen text-[10px]"></i>
                        </button>
                        <button onclick="deleteRecurring({{ $rec['id'] }}, this)"
                            class="w-7 h-7 rounded-lg bg-stone-100 dark:bg-stone-700 text-stone-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 flex items-center justify-center transition-colors" title="Hapus">
                ...
                <div class="flex gap-1 mb-3">
                    ...
                    <span class="w-7 h-7 rounded-full text-[10px] font-bold flex items-center justify-center flex-shrink-0
                ...
                <div class="mb-3 text-xs text-stone-500 dark:text-stone-400">Setiap tanggal <strong class="text-stone-800 dark:text-white">{{ $rec['day_of_month'] }}</strong></div>
                @else
                <div class="mb-3 text-xs text-stone-500 dark:text-stone-400">Setiap hari dalam periode aktif</div>
                @endif
                ...
                <div class="mb-3">
                    ...
                    <div class="w-full bg-stone-100 dark:bg-stone-700 rounded-full h-2">
                        <div class="h-2 rounded-full" style="width:{{ $pPct }}%;background:{{ $rec['color'] }}"></div>
                    </div>
                ...
                <div class="space-y-1.5 pt-2.5 border-t border-stone-100 dark:border-stone-800">
                    ...
                    <div class="flex items-center gap-2 text-xs text-stone-500 dark:text-stone-400">
                        <i class="fa-solid fa-clock text-[10px] w-3 text-center" style="color:{{ $rec['color'] }}"></i>
                        {{ $rec['start_time'] }} – {{ $rec['end_time'] }}
                    </div>
                    @endif
                    @if($rec['notes'])
                    <div class="flex items-start gap-2 text-xs text-stone-500 dark:text-stone-400">
                        <i class="fa-solid fa-circle-info text-[10px] w-3 text-center mt-0.5" style="color:{{ $rec['color'] }}"></i>
                        <span>{{ Str::limit($rec['notes'], 60) }}</span>
                    </div>
                    @endif
                    <div class="flex items-center gap-2 text-xs text-stone-500 dark:text-stone-400">
                        <i class="fa-solid fa-tag text-[10px] w-3 text-center" style="color:{{ $rec['color'] }}"></i>
...
{{-- ══════════════════════════════════════════════════════════════════
     MODAL: DAY DETAIL
══════════════════════════════════════════════════════════════════ --}}
<div id="modal-day-detail" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center p-4 flex">
    <div class="bg-white dark:bg-stone-900 rounded-2xl w-full max-w-md shadow-2xl border border-stone-200 dark:border-stone-800">
        <div class="flex items-center justify-between p-5 border-b border-stone-100 dark:border-stone-800">
            ...
            <button onclick="closeModal('modal-day-detail')" class="text-stone-400 hover:text-stone-700"><i class="fa-solid fa-xmark text-xl"></i></button>
        </div>
        ...
        <div class="p-5 pt-0">
            <button onclick="closeModal('modal-day-detail')" class="w-full py-2.5 bg-stone-800 dark:bg-stone-700 hover:bg-stone-900 text-white rounded-xl text-sm font-medium transition-colors">Tutup</button>
        </div>
    </div>
</div>
...
══════════════════════════════════════════════════════════════════ --}}
<div id="modal-add-event" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center p-4 flex">
    <div class="bg-white dark:bg-stone-900 rounded-2xl w-full max-w-lg shadow-2xl border border-stone-200 dark:border-stone-800 max-h-[95vh] overflow-y-auto">
        <div class="flex items-center justify-between p-6 border-b border-stone-100 dark:border-stone-800">
            <div>
                <h3 class="font-bold text-stone-900 dark:text-white">Tambah Event Kalender</h3>
                <p class="text-xs text-stone-400 mt-0.5">Event satu kali pada tanggal tertentu</p>
            </div>
            <button onclick="closeModal('modal-add-event')" class="text-stone-400 hover:text-stone-700 flex-shrink-0"><i class="fa-solid fa-xmark text-xl"></i></button>
        </div>
        ...
        <form id="form-add-event" novalidate>
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="title" id="ev-title" class="fi" placeholder="Nama event, misal: UTS Pengolahan Citra" required>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" id="ev-date" class="fi" value="{{ now()->format('Y-m-d') }}" required>
                        <p class="err-msg" id="ev-date-err">Pilih tanggal</p>
                    </div>
                </div>

                {{-- JAM: time picker, bukan string bebas --}}
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="fi-label">Jam Mulai <span class="text-rose-400">*</span></label>
                        <input type="time" name="start_time" id="ev-start-time" class="fi" value="08:00" required>
                        <p class="err-msg" id="ev-start-err">Masukkan jam mulai</p>
                    </div>
                    <div>
                        <label class="fi-label">Jam Selesai <span class="text-rose-400">*</span></label>
                        <input type="time" name="end_time" id="ev-end-time" class="fi" value="10:00" required>
                        <p class="err-msg" id="ev-end-err">Masukkan jam selesai</p>
                    </div>
                </div>
                <p class="text-[11px] text-stone-400">
                    <i class="fa-solid fa-circle-info text-stone-300 mr-1"></i>
                    Pilih jam mulai dan jam selesai menggunakan time picker di atas.
                </p>

                <div>
                    <label class="fi-label">Deskripsi / Catatan</label>
                    <textarea name="description" id="ev-notes" rows="2" class="fi resize-none" placeholder="Catatan tambahan, lokasi, dll..."></textarea>
                </div>

                <div class="flex items-center gap-2 p-3 bg-stone-50 dark:bg-stone-800 rounded-xl">
                    <input type="checkbox" name="is_all_day" id="ev-allday" value="1" class="w-4 h-4 accent-orange-500">
                    <label for="ev-allday" class="text-sm text-stone-600 dark:text-stone-300 cursor-pointer">Event seharian (All Day)</label>
                </div>
            </div>
            <div class="flex gap-3 px-6 pb-6">
                ...
                <button type="button" onclick="submitEvent(this)"
                    ...
                    <i class="fa-solid fa-calendar-plus"></i> Simpan Event
                </button>
            </div>
        </form>
    </div>
...
══════════════════════════════════════════════════════════════════ --}}
<div id="modal-add-recurring" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center p-4 flex">
    <div class="bg-white dark:bg-stone-900 rounded-2xl w-full max-w-lg shadow-2xl border border-stone-200 dark:border-stone-800 max-h-[95vh] overflow-y-auto">
        <div class="flex items-center justify-between p-6 border-b border-stone-100 dark:border-stone-800">
            <div>
                <h3 class="font-bold text-stone-900 dark:text-white" id="rec-modal-title">Tambah Kegiatan Rutin</h3>
                <p class="text-xs text-stone-400 mt-0.5">Kegiatan berulang harian, mingguan, atau bulanan</p>
            </div>
            <button onclick="closeModal('modal-add-recurring')" class="text-stone-400 hover:text-stone-700 flex-shrink-0"><i class="fa-solid fa-xmark text-xl"></i></button>
        </div>
        ...
        <div class="p-6 space-y-4">
            <div>
                ...
                <input type="text" id="rec-title" class="fi" placeholder="misal: PKL Full Day, Olahraga Pagi, Kuliah IF401">
            ...
            <div id="weekly-opts">
                ...
                <div class="flex gap-1.5">
                    ...
                    <button type="button" onclick="toggleDay({{ $di }}, this)" id="day-btn-{{ $di }}"
            ...
            <div id="monthly-opts" class="hidden">
                ...
                <div class="flex items-center gap-3">
                    <input type="number" id="rec-dom" min="1" max="31" placeholder="contoh: 15" class="fi w-32">
            ...
            <div class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-[11px] text-blue-700 dark:text-blue-400 flex items-start gap-2">
                <i class="fa-solid fa-circle-info mt-0.5 flex-shrink-0"></i>
                <span>Pilih jam menggunakan time picker. Contoh: olahraga jam 06:00–07:00, PKL jam 08:00–17:00.</span>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" id="rec-end" class="fi" value="{{ now()->addMonths(6)->format('Y-m-d') }}">
...
function toast(msg, ok = true) {
    ...
    t.className = `fixed bottom-6 right-6 z-[9999] flex items-center gap-2 px-4 py-3.5 ${ok?'bg-gradient-to-r from-emerald-500 to-emerald-600':'bg-gradient-to-r from-rose-500 to-rose-600'} text-white text-sm font-semibold rounded-2xl shadow-2xl`;
...
function filterRecurring(type) {
    const on  = 'px-3 py-1.5 text-xs rounded-full font-medium bg-stone-800 dark:bg-stone-700 text-white transition-colors';
    const off = 'px-3 py-1.5 text-xs rounded-full font-medium bg-stone-100 dark:bg-stone-800 text-stone-500 hover:bg-stone-200 transition-colors';
...
function toggleDay(d, btn) {
    const activeClass   = 'flex-1 py-2 rounded-xl text-xs font-bold border-2 border-orange-400 text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 transition-all';
    const inactiveClass = 'flex-1 py-2 rounded-xl text-xs font-bold border-2 border-stone-200 dark:border-stone-700 text-stone-400 hover:border-orange-400 hover:text-orange-500 transition-all';
...
function showDayDetail(dateStr) {
    ...
    if (!oneOff.length && !recs.length) {
        body.innerHTML = '<div class="text-center py-8"><i class="fa-regular fa-calendar text-4xl text-stone-300 mb-3 block"></i><p class="text-stone-400 text-sm">Tidak ada jadwal hari ini.</p></div>';
    } else {
        let html = '';
        if (recs.length) {
            html += '<p class="text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-2">Kegiatan Rutin</p>';
            html += recs.map(e => `
        ...
        if (oneOff.length) {
            ...
            html += '<p class="text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-2">Event Khusus</p>';
            html += oneOff.map(e => `
                <div class="flex items-center gap-3 p-2.5 bg-stone-50 dark:bg-stone-800 rounded-xl mb-2">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style="background:${e.color}">
                        <i class="fa-solid fa-calendar-day text-white text-xs"></i></div>
                    <div class="min-w-0 flex-1">
                        <p class="text-sm font-semibold text-stone-800 dark:text-white truncate">${e.title}</p>
...
async function submitEvent(btn) {
    ...
    let valid = true;
    if (!title)  { showErr('ev-title-err'); valid = false; }
    if (!date)   { showErr('ev-date-err');  valid = false; }
    if (!startT) { showErr('ev-start-err','Masukkan jam mulai'); valid = false; }
    if (!endT)   { showErr('ev-end-err','Masukkan jam selesai'); valid = false; }
    if (!valid) return;
    ...
    try {
        ...
        if (data.success) {
            toast(data.message || 'Event berhasil ditambahkan!');
...
async function submitRecurring(btn) {
    clearAllErr('rec-title-err','rec-days-err','rec-dom-err','rec-stime-err','rec-etime-err','rec-start-err','rec-end-err');
    ...
    let valid = true;
    if (!title) { showErr('rec-title-err'); valid = false; }
    if (!startT) { showErr('rec-stime-err','Masukkan jam mulai'); valid = false; }
    if (!endT)   { showErr('rec-etime-err','Masukkan jam selesai'); valid = false; }
    if (!startD) { showErr('rec-start-err','Pilih tanggal mulai'); valid = false; }
    if (!endD)   { showErr('rec-end-err','Pilih tanggal berakhir'); valid = false; }
    if (freq === 'weekly' && selectedDays.size === 0) { showErr('rec-days-err','Pilih minimal satu hari'); valid = false; }
    if (freq === 'monthly' && (!dom || dom < 1 || dom > 31)) { showErr('rec-dom-err','Masukkan tanggal 1-31'); valid = false; }
    if (!valid) return;
...
function editRecurring(id) {
    ...
    if (rec.frequency === 'weekly' && rec.days.length) {
        rec.days.forEach(d => {
            ...
            if (btn) btn.className = 'flex-1 py-2 rounded-xl text-xs font-bold border-2 border-orange-400 text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 transition-all';
        });
    }
...
async function deleteRecurring(id, btn) {
    if (!confirm('Hapus kegiatan rutin ini? Data tidak bisa dikembalikan.')) return;
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            if (card) { card.style.transition='all .25s'; card.style.opacity='0'; setTimeout(()=>card.remove(),250); }
...
function resetRecurringForm() {
    ...
    for (let d = 0; d <= 6; d++) {
        ...
        if (b) b.className = 'flex-1 py-2 rounded-xl text-xs font-bold border-2 border-stone-200 dark:border-stone-700 text-stone-400 hover:border-orange-400 hover:text-orange-500 transition-all';
    }
    // Reset color
    selectedRecColor = '#10b981';
    document.querySelectorAll('[data-color]').forEach(b => { b.style.outline='none'; b.style.borderColor='transparent'; });
    // Reset modal title
