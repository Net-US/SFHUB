...
@section('content')
    ...
    <div class="fade-up space-y-5">
        ...
        @if ($pklInfoArr['company'] === '-')
            <div
                class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-5 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-amber-700 dark:text-amber-500 text-xs mt-0.5">Isi informasi perusahaan dan jadwal PKL
                        terlebih dahulu agar fitur ini berfungsi penuh.</p>
                    <button onclick="openModal('modal-edit-pkl')"
                        class="mt-2 px-4 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-medium rounded-xl transition-colors">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            @foreach ([[number_format($hoursDone, 1), 'Jam Selesai', 'fa-clock', 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400'], [$pklInfoArr['hours_required'], 'Jam Wajib', 'fa-hourglass-half', 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'], [$pctDone . '%', 'Progres Jam', 'fa-chart-pie', 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400'], [$daysLeft, 'Hari Tersisa', 'fa-calendar-days', 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400']] as [$v, $l, $ic, $cls])
                <div
        ...
        <div id="pkl-log" class="pkl-pane space-y-4">
            @if (empty($activities))
                <div
                    ...
                    <button onclick="openModal('modal-log-activity')"
                        class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-medium transition-colors">
                ...
                <div
                    ...
                    <div class="divide-y divide-stone-100 dark:divide-stone-800">
                        @foreach ($activities as $act)
                            ...
                            <div class="flex items-start gap-4 px-6 py-4 hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors group"
                                id="act-row-{{ $act['id'] }}">
                                <div
                                    class="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center flex-shrink-0 mt-0.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-0.5 flex-wrap">
                                        ...
                                        <span
                                            class="text-[10px] px-2 py-0.5 rounded-full font-medium {{ $catColor }}">{{ $act['category'] }}</span>
                                    </div>
                                    @if ($act['notes'])
                                ...
                                <button onclick="deleteActivity({{ $act['id'] }}, this)"
                                    class="w-8 h-8 rounded-lg bg-stone-100 dark:bg-stone-700 text-stone-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all"
                                    title="Hapus">
        ...
        <div id="pkl-jadwal" class="pkl-pane hidden space-y-4">
            <div
                ...
                <div class="px-6 py-4 border-b border-stone-100 dark:border-stone-800 flex items-center justify-between">
                    ...
                    <button onclick="openModal('modal-edit-schedule')"
                        class="text-xs text-emerald-600 hover:text-emerald-700 flex items-center gap-1 font-medium transition-colors">
                ...
                <div class="divide-y divide-stone-100 dark:divide-stone-800">
                    @foreach ($schedule as $s)
                        ...
                        <div class="flex items-center gap-4 px-6 py-4">
                                ...
                                @else
                                    ...
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        @if ($s['has_split'] ?? false)
                                            <span
                                                class="text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full font-bold">Sesi
                                                1</span>
                                        @endif
                                        <span class="text-[11px] text-stone-400">({{ number_format($s['total_hours'], 1) }}
                                            jam total)</span>
                                    </div>
                                    {{-- Sesi 2 (split) --}}
                                    @if ($s['has_split'] ?? false)
                            ...
                            <span
                                class="text-[10px] px-2 py-0.5 rounded-full font-semibold {{ $typeCls }} flex-shrink-0">{{ $typeLabel }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('pkl.activity.store') }}" id="form-log-activity">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" name="task" id="la-task" class="fi"
                            placeholder="Apa yang dikerjakan hari ini?" required>
                    </div>
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            <label class="fi-label">Tanggal <span class="text-rose-400">*</span></label>
                            <input type="date" name="log_date" id="la-date" class="fi"
                                value="{{ now()->format('Y-m-d') }}" required>
                        </div>
                        <div>
                            <label class="fi-label">Jam Kerja <span class="text-rose-400">*</span></label>
                            <input type="number" name="hours" id="la-hours" min="0.5" max="24"
                                step="0.5" class="fi" placeholder="4" required>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="la-notes" rows="3" class="fi resize-none"
                            placeholder="Hasil pekerjaan, hambatan, catatan untuk laporan..."></textarea>
                    </div>
                </div>
                <div class="flex gap-3 px-6 pb-6">
                    <button type="button" onclick="closeModal('modal-log-activity')"
        ...
        <div
            ...
            <form method="POST" action="{{ route('pkl.schedule.update') }}" id="form-edit-schedule">
                ...
                <div class="p-6 space-y-3">
                    ...
                    {{-- Panduan split-shift --}}
                    <div
                        class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-[11px] text-blue-700 dark:text-blue-400 flex items-start gap-2 mb-2">
                        <i class="fa-solid fa-circle-info flex-shrink-0 mt-0.5"></i>
                        <span><strong>Split Shift:</strong> Pilih tipe "Split Shift" lalu isi <strong>Sesi 1</strong> (misal
                            08:00–09:00) dan <strong>Sesi 2</strong> (misal 13:00–16:00) untuk jadwal kuliah di tengah hari.
                            Total jam dihitung otomatis.</span>
                    </div>

                    @foreach ($schedule as $i => $s)
                        @php $isOff = $s['type'] === 'off'; @endphp
                        <div class="bg-stone-50 dark:bg-stone-800 rounded-xl p-4 space-y-3 border border-stone-200 dark:border-stone-700"
                            id="sched-row-{{ $i }}">
                            {{-- Row header: hari + tipe --}}
                            <div class="flex items-center gap-3">
                                <span
                                    class="w-20 text-sm font-bold text-stone-700 dark:text-stone-300 flex-shrink-0">{{ $s['day'] }}</span>
                                <input type="hidden" name="schedules[{{ $i }}][day]"
                                    value="{{ $s['day'] }}">
                                <select name="schedules[{{ $i }}][type]" id="type-{{ $i }}"
                                    onchange="toggleScheduleType({{ $i }}, this.value)" class="fi"
                                    style="width:auto;flex:1">
                                    <option value="full" {{ $s['type'] === 'full' ? 'selected' : '' }}>✅ Full Day</option>
                                    <option value="half" {{ $s['type'] === 'half' ? 'selected' : '' }}>🌤 Half Day</option>
                                    <option value="split" {{ $s['type'] === 'split' ? 'selected' : '' }}>⚡ Split Shift
                                    </option>
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="flex-1">
                                        ...
                                        <input type="time" name="schedules[{{ $i }}][start_time]"
                                            id="start1-{{ $i }}" value="{{ $s['start'] }}" class="fi"
                                            {{ $isOff ? 'disabled' : '' }}>
                                    </div>
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <input type="time" name="schedules[{{ $i }}][end_time]"
                                            id="end1-{{ $i }}" value="{{ $s['end'] }}" class="fi"
                                            {{ $isOff ? 'disabled' : '' }}>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div id="sesi2-{{ $i }}" class="{{ $s['type'] !== 'split' ? 'hidden' : '' }}">
                                <div class="split-row space-y-2">
                                    <p
                                        ...
                                        ⚡ Sesi 2 — Lanjut setelah jeda (misal: setelah kuliah)</p>
                                    <div class="flex items-center gap-3">
                                        <div class="flex-1">
                                            <label class="fi-label text-[10px]">Jam Mulai Sesi 2</label>
                                            <input type="time" name="schedules[{{ $i }}][start_time_2]"
                                                id="start2-{{ $i }}" value="{{ $s['start2'] ?? '' }}"
                                                class="fi">
                                        </div>
                                        <span class="text-stone-400 text-sm mt-5 flex-shrink-0">–</span>
                                        <div class="flex-1">
                                            <label class="fi-label text-[10px]">Jam Selesai Sesi 2</label>
                                            <input type="time" name="schedules[{{ $i }}][end_time_2]"
                                                id="end2-{{ $i }}" value="{{ $s['end2'] ?? '' }}"
                                                class="fi">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div id="notes-{{ $i }}" class="{{ $isOff ? 'hidden' : '' }}">
                                ...
                                <input type="text" name="schedules[{{ $i }}][notes]"
                                    value="{{ $s['notes'] ?? '' }}" class="fi text-xs"
                                    placeholder="misal: WFH, tugas lapangan, dll">
                            </div>
                        </div>
                    @endforeach
                ...
                <div class="flex gap-3 px-6 pb-6">
                    <button type="button" onclick="closeModal('modal-edit-schedule')"
                        class="flex-1 py-2.5 border border-stone-300 dark:border-stone-700 rounded-xl text-stone-600 dark:text-stone-300 text-sm hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors">Batal</button>
                    <button type="submit"
                        class="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-semibold transition-colors">
            ...
            <form method="POST" action="{{ route('pkl.info.store') }}" id="form-edit-pkl">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" name="company" id="mc-company"
                            value="{{ $pklInfoArr['company'] !== '-' ? $pklInfoArr['company'] : '' }}" class="fi"
                            placeholder="PT. Nama Perusahaan" required>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="fi-label">Departemen</label>
                            <input type="text" name="department" id="mc-dept"
                                value="{{ $pklInfoArr['department'] !== '-' ? $pklInfoArr['department'] : '' }}"
                                class="fi" placeholder="IT / Marketing / Desain">
                        </div>
                        <div>
                            <label class="fi-label">Nama Supervisor</label>
                            <input type="text" name="supervisor" id="mc-supervisor"
                                value="{{ $pklInfoArr['supervisor'] !== '-' ? $pklInfoArr['supervisor'] : '' }}"
                                class="fi" placeholder="Bapak/Ibu ...">
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="fi-label">HP Supervisor</label>
                            <input type="text" name="supervisor_phone" id="mc-supervisor-hp"
                                value="{{ $pklInfoArr['supervisor_hp'] !== '-' ? $pklInfoArr['supervisor_hp'] : '' }}"
                                class="fi" placeholder="08xx-xxxx-xxxx">
                        </div>
                        <div>
                            <label class="fi-label">Tunjangan/Bulan (Rp)</label>
                            <input type="number" name="allowance" id="mc-allowance"
                                value="{{ $pklInfoArr['allowance'] }}" class="fi" placeholder="0">
                        </div>
                    </div>
                    <div>
                        <label class="fi-label">Alamat Perusahaan</label>
                        <input type="text" name="address" id="mc-address"
                            value="{{ $pklInfoArr['address'] !== '-' ? $pklInfoArr['address'] : '' }}" class="fi"
                            placeholder="Jl. ...">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="fi-label">Tanggal Mulai PKL <span class="text-rose-400">*</span></label>
                            <input type="date" name="start_date" id="mc-start"
                                value="{{ $pklInfoArr['start_date'] }}" class="fi">
                        </div>
                        <div>
                            <label class="fi-label">Tanggal Selesai PKL <span class="text-rose-400">*</span></label>
                            <input type="date" name="end_date" id="mc-end" value="{{ $pklInfoArr['end_date'] }}"
                                class="fi">
                        </div>
                    </div>
                    <div>
                        <label class="fi-label">Total Jam Wajib</label>
                        <input type="number" name="hours_required" id="mc-hours"
                            value="{{ $pklInfoArr['hours_required'] }}" class="fi" placeholder="720"
                            min="1">
                        <p class="text-[10px] text-stone-400 mt-1">Contoh: 720 jam = ±6 bulan PKL</p>
                    </div>
                </div>
                <div class="flex gap-3 px-6 pb-6">
                    <button type="button" onclick="closeModal('modal-edit-pkl')"
                        class="flex-1 py-2.5 border border-stone-300 dark:border-stone-700 rounded-xl text-stone-600 dark:text-stone-300 text-sm hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors">Batal</button>
                    <button type="submit"
                        class="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-semibold transition-colors">
...
@push('scripts')
    <script>
        ...
        async function deleteActivity(id, btn) {
            ...
            const title = row?.querySelector('h4')?.textContent?.trim() || 'Aktivitas ini';

            showDeleteConfirm({
                title: 'Hapus Aktivitas?',
                message: `Hapus "${title}" dari log aktivitas?`,
                warning: 'Data aktivitas yang dihapus tidak dapat dikembalikan.',
                btnRef: btn,
                onConfirm: async () => {
